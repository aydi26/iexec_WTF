...
- The aggregate `A` is public (it's the CCTP burn amount). Privacy requires **k ≥ 3 depositors/epoch**; `settleEpoch` reverts below `minDepositors` (demo: 3). Three privacy tiers, stated honestly: **(a)** default — the k=3 floor is met by two operator fillers whose amounts are now **randomized (0.2–0.7) and encrypted**, so an external observer can no longer recover your amount as `A − constant`; only the operator, who drew the fillers, still knows it. **(b)** **Pool mode** (settings toggle) — no fillers; the batch waits for two **independent** depositors, so your amount is hidden among three unknowns *even from the operator* (**real k=3**). **(c)** With organic traffic, concurrent independent users fill each other's batches automatically. (b)/(c) are genuine amount-hiding; (a) is convenience with a residual k=1-vs-operator.
