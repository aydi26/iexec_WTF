...
- **Live + Sourcify-verified deployments (current — redeployed 2026-07-22 with batch entry points `preRegisterMany`/`depositMany` + the F-9 CEI fix in `withdrawDeposit`; bidirectional, third-party sends enabled):** cUSDC ETH `0x47d150572dFCEB75C27b6dDf5EADc4D6fa33e41C` · Arb `0xD74A1F2bF0285Dc64F7855D0233E774772Ab0209`. **ETH→Arb**: Batcher@ETH `0x1a3534aBA28229392b881DC9f226F3b907a2b7FD`, Distributor@Arb `0xF67242Fc24da75AD25c8B80b9FcfACbd9b5Ec499`. **Arb→ETH**: Batcher@Arb `0x76A6507c819500Cc8793f70f8735Be5864B1dea3`, Distributor@ETH `0x10c2430b89CeCBE08aabEA57E88AEC76B95F9c3f`. All 6 Sourcify `exact_match`. (Earlier redeploy sets appear only in past worklog entries.)
- **Live app + keeper (2026-07-22):** https://iexecwtf.vercel.app — Vercel builds from the REPO ROOT (root `vercel.json`); serverless keeper at `/api/keeper` (source `api/keeper.mjs`, address `0x50ea6bF3D5e8B5F6A7b9Cc1842B09EfE01851abC`, gas-only key — steps gated by state + Circle attestation + KMS proof, never caller identity). Keeper phases: ping / fill / close / settle / attest / relaycheck / finalize; **fill** = operator contributes two **randomized encrypted fillers (0.20–0.70 cUSD each)** from its own pre-wrapped liquidity within ~5 s (fillers cycle back to the operator on dst), proven live both directions. **Pool mode** (frontend toggle) drops fillers and waits for 2 independent depositors → real k=3 (proven live, `scripts/10_pool_k3_proof.ts`). User signatures: recurring ≈ 2 (`preRegisterMany` + `depositMany`, batch E2E proven live on the new set — A=1050000, check==1, state 4); first bridge adds one-time funding (approve max + wrap + setOperator) ≈ 5. Fallbacks: keeper unreachable → client-side signing (~7 with contract batching); fill can't engage → self-filler mode; EIP-5792 atomic batching on smart-account wallets. All 4 flows (honest fwd/rev, third-party, adversarial) proven live on the current set. Product rules: 1 USDC/bridge cap, k=3 floor.
...
#### Session 20 — 2026-07-24 — claude — Post-audit fix pass: burn-tx recovery repaired in prod + keeper liquidity topped up
**Done (all S19 warnings closed except user-only items):** (1) `api/keeper.mjs settledTxHash` rewritten — walks logs-capable RPC fallbacks after the configured RPC (publicnode 403s wide `getLogs`, Alchemy free caps at 10 blocks), wide-range then newest-first chunked scan, per-chain window/chunk (ETH 50k/9.5k ≈ 7 days; **Arb widened to 600k/45k** — 50k was only hours of Arb blocks), 20s deadline. (2) `bridge.js recoverSettleTx` client-side mirror through the app's own RPC transport when the keeper returns no hash (each side covers the other's blind spots). (3) Dead Satoshi `@font-face` removed (files never existed — kills the runtime 404s, zero visual change). (4) **Keeper filler liquidity decrypted with its own key** (first read-back ever): ETH 1.48 / Arb 9.52 cUSD → topped up ETH via deployer `wrap(keeper, 8 USDC)` tx `0xc98e11…9502` → **ETH 9.48 / Arb 9.52** (~6 worst-case bridges per side). (5) SPEC §14 team/contact placeholders filled.
**Verified (LIVE, prod):** after Vercel redeploy (commit 391875c), `phase=settle` on the two Settled epochs returns the REAL burn txs — eth-arb epoch 8 → `0x28f56e…6462`, arb-eth epoch 1 → `0xdbc15c…ec0f` (both were `hash:null` before the fix) → **Track-tab Resume repaired in production**. Build vert + 31/31 tests + `node --check` keeper.
**Open (user-only):** ≤4-min video + X post @iEx_ec · rotate the Alchemy key (echoed into a LOCAL session transcript by an ethers error — never in the repo) and update local `.env` + Vercel env.
**feedback.md candidates:** none new.

#### Session 19 — 2026-07-24 — claude — Final pre-submission audit: fresh bidirectional E2E + 8-agent readiness sweep (0 blockers)
**Done:** frontend polish shipped (commit 41b0450: team-card avatar `aiden.png` 512², widget title "Exchange" → "Bridge confidential token via CCTP", 2-row widget header). **Repo made PUBLIC by user** (HTTP 200 logged out — was the #1 submission blocker). Ran an 8-agent submit-readiness workflow: fresh live E2E both directions + unit tests + build/config + secrets/hygiene + compliance + on-chain readiness + historical evidence + adversarial completeness critic → **0 blockers**.
**Verified (LIVE, 2026-07-24):** (1) fresh honest E2E **ETH→Arb** — epoch 8, A=0.10, settle+burn `0x28f56e…6462`, relay `0xc7125f…2dce`, finalize `0xa09667…ece9`, Batcher Settled(2) + Distributor Distributed(4), aggregates match, check==1; (2) fresh honest E2E **Arb→ETH** — epoch 1, A=0.10, burn `0xdbc15c…ec0f`, relay `0x0da6c7…06a3`, finalize `0x8bee01…1598`, Settled+Distributed, check==1. 31/31 unit tests; vite build clean; audit:privacy PASS (3/3 sites); secrets scan clean (no key fragments/private keys in tracked files or history; `.env` never committed); README deploy table == `deployments/*.json` == frontend/api copies (byte-identical); feedback.md substantive; 6/6 Sourcify (5 exact_match, Arb cUSDC runtime match); buffers ETH-B 3.10 / ETH-D 5.00 / Arb-B 3.00 / Arb-D 5.00 USDC; keeper gas 0.123/0.140 ETH; **zero stuck epochs** (ETH 0–8 terminal + 9 fresh Open; Arb 0–1 terminal + 2 fresh Open); historical modalities re-confirmed on-chain (pool k=3 epoch 1 with 3 distinct non-keeper wallets; adversarial epoch 3 Refunded+RefundInitiated; real-browser epochs 6/7 user+friend; reverse Arb epoch 0). Live Vercel app serves the HEAD commit (bundle string probe) + keeper ping OK.
**Open (user-only):** ≤4-min video (script: docs/DEMO_SCRIPT.md) + X post @iEx_ec (draft: docs/X_POST.md) — then submit.
**Warnings (non-blocking, logged):** deployed keeper `settledTxHash` getLogs returns null on default RPCs (probed live: phase=settle on Settled epochs → `hash:null` both directions) → Track-tab **Resume fails only in the narrow Settled-but-unrelayed window**; fix = logs-capable `ETH/ARB_SEPOLIA_RPC_URL` in Vercel env (note: Alchemy free tier caps getLogs at 10 blocks — needs a range-tolerant endpoint or code chunking). Alchemy key was echoed into the LOCAL session transcript by an ethers error (`requestUrl`) — not in the repo; rotation recommended. Keeper plain USDC = 0 both chains (filler liquidity is encrypted cUSDC — one frontend dry-run recommended before recording). `04_honest_e2e.ts` stale banner "(0.45 USDC)" fixed this session. Satoshi fonts 404 (system-font fallback, cosmetic).
**feedback.md candidates:** none new.
